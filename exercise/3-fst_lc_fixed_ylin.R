library(rjdfilters)
library(zoo)
fst_f <- readRDS("comparison_filters/filters/fst.RDS")
fst_f <- lapply(fst_f, `[[`,"lc")
...
dir_exp <- "comparison_filters/fst/"
i <- 9
for(i in 1:5){
    ...
    series_s <- lapply(data, function(x_all){
        ...
        lapply(names(x_all), function(date_fin){
            ...
            filter <- fst_f[[sprintf("h=%i", horizon)]]
    ...
    nom_f_s <- sprintf("%sfst_lc_%02.f_fixedylin.RDS", dir_exp, i)
    ...
    saveRDS(tp_rkhs,
            sprintf("%sfst_lc_tp_%02.f_fixedylin.RDS", dir_exp, i))
}
