# Regression polynomiale locale {#sec:lppfilters}
...
Le filtre DAF $v$ et les prévisions $\hat{y}_{f}$ peuvent s'écrire :
$$
\theta_{a}=K_{p}X_{p}(X'_{p}K_{p}X_{p})^{-1}e_{1},
\quad
\hat{y}_{f}=X_{f}(X'_{p}K_{p}X_{p})^{-1}X_{p}'K_{p}y_{p}
$$
De plus, on a les propriétés suivantes sur $v$ :

- il préserve les tendances polynomiales de degré $d$ comme le filtre symétrique. 

- $v$ minimise la distance pondérée (par le noyau) entre les coefficients du filtre asymétrique et ceux du filtre symétrique.
Cela prouve donc l'équivalence entre les trois méthodes.

L'inconvénient de cette méthode est que les poids de $\theta_{a,0}$ sont fortement concentrés sur l'estimation courrante, avec une révision importante entre $q=0$ (filtre en temps réel) et $q=h$ (filtre symétrique, voir \@ref(fig:graphsdaf)). 
...
La table \@ref(tab:criteriaLp) compare les critères de qualité des différentes méthodes en utilisant le filtre d'Henderson et $h=6$ (filtre symétrique de 13 termes). 
...
Une application en ligne, disponible à l'adresse https://aqlt.shinyapps.io/FiltersProperties/, permet de comparer compare les coefficients, les fonctions de gain et de déphasage entre les différentes méthodes et les différents noyaux.
...
$$
\E{\left( \sum_{i=-h}^h\theta_sy_{t+s}-\sum_{i=-h}^qvy_{t+s} \right)^2}
= I(v,\,0,\,y_t,\,M_\theta y_t)
$$
...
Cette modélisation leur permet de prendre en compte dans l'estimation de la tendance la structure de corrélation induite par le plan de sondage de l'enquête emploi mensuelle de l'Australie (groupe de rotations avec une période de recouvrement).
...
Étant donné un filtre symétrique $\theta_s$ utilisé pour estimer la tendance au centre de la série, l'objectif est de chercher un filtre asymétrique $v=(\theta_{-h},\dots,\theta_q)$ de sorte à minimiser l'erreur quadratique moyenne de révision :
$$
...
1. Dans le premier cas ils cherchent un estimateur sans biais : cela implique que $v$ conserve les mêmes tendances polynomiales que $\theta_s$. 
...
La théorie générale définie dans la section \@ref(subsec:theoriegen) permet également de retrouver les filtres asymétriques puisqu'ils sont construits en minimisant l'erreur quadratique moyenne des révisions sous contraintes linéaires (preservation d'un polynôme de degré $p$)
...
Comme discuté dans @GrayThomson1996, cette méthode n'est pas étudiée pour trois raisons :

- Il n'est pas clair qu'il faudrait chercher à maintenir la même équilibre entre *smoothness* et *fidelity* en fin de série et au centre de la série.
...
### Liens entre les critères de Gray et Thomson et ceux de Guggemos *et ali*

Les critères $F_g$ et $S_g$ peuvent se déduire de $F_{GT}$ et $S_{GT}$.
Les approches de @GrayThomson1996 et @ch15HBSA sont donc équivalentes pour le filtre symétrique.
...
L'équivalence entre la méthode de @ch15HBSA et celle de @proietti2008 peuvent donc se voir comme un cas particulier de l'équivalence entre les moindres carrés pondérés et les moindres carrés généralisés.
C'est par exemple le cas des filtres symétriques d'Henderson qui peuvent s'obtenir par les deux méthodes.

Dans ce sens, @henderson1916note a montré que les poids $w=(w_{-p},\dots w_{f})$ de associés à la moyenne mobile issue de la régression polynomiale locale par moindres carrés pondérés pouvaient s'écrire sous la forme :
$$
w_i = \kappa_i P\left(\frac{i}{p+f+1}\right)\text{ où }P\text{ est un polynôme de degré }d.
$$
Il a également montré l'inverse : toute moyenne mobile $\theta=(\theta_{-p},\dots \theta_{f})$ qui préserve les tendances de degré $d$ et donc le diagramme des coefficients change au plus $d$ fois de signes peut être obtenu par une régression polynomiale locale de degré $p$ estimée par moindres carrés pondérés. 
...
C'est le cas de tous les filtres symétriques issues de l'approche FST et de la majorité des filtres asymétriques (voir annexe \@ref(an:equivfstlp)). 


Plus récemment @LuatiProietti2011 se sont intéressés aux cas d'équivalences entre les moindres carrés pondérés et les moindres carrés généralisés d'équivalences pour déterminer des les noyaux optimaux (au sens de Gauss-Markov).
Ils montrent que le noyau d'Epanechnikov est le noyau optimal associé à la régression polynomiale locale où le résidus, $\varepsilon_t$, est un processus moyenne mobile (MA) non inversible d'ordre 1 (i.e. : $\varepsilon_t=(1-B)\xi_t$ avec $\xi_t$ un bruit blanc). 
...
De même, le noyau d'Henderson est le noyau optimal associé à la régression polynomiale locale où le résidus est un processus moyenne mobile (MA) non inversible d'ordre 2 (i.e. : $\varepsilon_t=(1-B)^2\xi_t$ avec $\xi_t$ un bruit blanc).
