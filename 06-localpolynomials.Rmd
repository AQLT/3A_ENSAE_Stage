...
```{r filtersdafcoefs, fig.cap="Coefficients et fonctions de gain des filtres directs asymétriques (DAF) computed by local polynomial of degree $3$ with the Henderson kernel for $h=6$.", echo = FALSE, out.width="100%"}
...
Inversement, @ch15HBSA justifie de ne pas intégrer le critère de révision dans leur problème car ce critère est fortement corrélé à une combinaison fixée
ne considère pas ici le critère de révision dont on peut en fait prouver qu'il est très fortement corrélé à une combinaison 􏰀xée, donc non ajustable par l'utilisateur, des critères de Fidélité et de Temporalité
::::
...
En effet, dans ce cas la solution est $\hat \theta = \Sigma^{-1}X_p'\left(X_p\Sigma^{-1}X_p'\right)^{-1}e_1$ avec $\Sigma=\alpha F+\beta S+ \gamma T$, et c'est l'estimation de la constante obtenue par moindres carrés généralisés lorsque la variance des résidus est $\Sigma^{-1}$.
