# D'une théorie générale sur la construction des filtres asymétriques à l'approche FST

## Théorie générale de construction des filtres asymétriques {#subsec:theoriegen}

Pour tenter établir une théorie générale englobant les principaux filtres linéaires, @ch15HBSA définissent deux critères :
\begin{align}
I(\theta,q,y_t,u_t)&=\E{(\Delta^{q}M_\theta y_t-u_t)^{2}} (\#eq:theoriegen1) \\
J(\theta,f, \omega_1,\omega_2)&=\int_{\omega_1}^{\omega_2} f\left[\phi_\theta(\omega), \varphi_\theta (\omega)\right] \ud \omega (\#eq:theoriegen2)
\end{align}
...
$S_g$ peut également s'écrire sous une forme quadratique positive $S_g(\theta)=\theta'S\theta$ avec $S$ une matrice symétrique d'ordre $p+f+1$ (voir section XXXX).
...
C'est pourquoi le critère de déphasage dépend des fonctions de gain et de déphasage ($\rho_\theta$ et $\varphi_{\theta}$), le lien entre les deux fonctions étant faite à partir d'une fonction de pénalité $f$^[
@ch15HBSA suggèrent 6 conditions de régularité à la fonction de pénalité afin qu'elle soit adaptée au problème de déphasage : $f \geq 0$, $f\left(\rho,0\right)=0$, $f\left(0,\varphi\right)=0$, $f\left(\rho,\varphi\right)=f\left(\rho,-\varphi\right)$, $\frac{\partial f}{\partial \rho} \geq 0$ et
...
Cela permet notamment d'avoir une *timeliness* qui peut s'écrire comme une forme quadratique positive ($T_g(\theta)=\theta'T\theta$ avec $T$ une matrice carré symmétrique d'ordre $p+f+1$, voir @ch15HBSA pour la démonstration).
...
Les trois critères utilisés dans le programme de minimisation \@(eq:gugemmos) sont des cas particuliers de ceux définis dans la section \@(subsec:theoriegen). 
...
:::: {.summary_box data-latex="{Filtres FST}"}
`r if (is_html) '
:::{.title}
Filtres FST
:::
