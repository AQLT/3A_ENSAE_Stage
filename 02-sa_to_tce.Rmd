...
C'est pourquoi les séries chronologiques sont généralement corrigées des variations saisonnières et des jours de jours ouvrables, la *désaisonnalisation* étant le processus consistant à supprimer les effets des fluctuations saisonnières et des jours ouvrables.
Un effet saisonnier est un phénomène qui se répète chaque année autour de la période, avec une ampleur et une direction similaires d'une année à l'autre.
...
La tendance et le cycle n'étant pas observés et étant difficiles à séparer, elles sont estimées de manière conjointes dans la désaisonnalisation.
...
Ce lien explique également que toutes les méthodes utilisées dans ce rapport sont implémentées dans les bibliothèques de base de JDemetra+ ^[Voir https://github.com/jdemetra/jdemetra-core.], le logiciel de désaisonnalisation recommandé par Eurostat.
Une interface `r fa_r_project`, dévelopée au cours de ce stage, est implémentée dans le package `rjdfilters`^[Disponible sur https://github.com/palatej/rjdfilters.].
...
Les moyennes mobiles asymétriques classiques minimisent les erreurs de révision mais introduisent des retards dans la détection des points de retournement (déphasage, voir section \@ref(sec:propMM)).
...
- Les *Model-Based Approach*  --- approche basée sur les modèles --- supposent la spécification d'un modèle stochastique pour la tendance (modèle ARIMA, modèle d'espace d'état, etc.) et les estimations sont obtenues en minimisant une fonction de pénalité, généralement l'erreur quadratique moyenne.
...
C'est par exemple le cas des filtres @henderson1916note et @musgrave1964set (utilisés dans X-12ARIMA).
...
Bien que ces auteurs aient proposé des approches générales pour dériver des filtres linéaires, ils ne font référence qu'aux méthodes les plus classiques (Henderson, Musgrave, Hodrick-Prescott, etc.) sans faire le lien avec les autres méthodes récentes.
...
D'une part cela permet de rassembler une première revue de la littérature sur les méthodes de construction des filtres linéaires pour l'analyse conjoncturelle.
...
Pour cela nous utiliserons l'approche générale de @ch15HBSA (section \@ref(subsec:theoriegen)) qui ont également proposé une procédure globale pour construire des moyennes mobiles asymétriques, qui permet de minimiser les effets de déphasage(section \@ref(subsec:GuggemosEtAl)).
...
C'est pourquoi ni les filtres de l'approche de @trilemmaWMR2019 (section \@ref(sec:WildiMcLeroy)), ni d'autres approches basées sur des modèles ne sont, pour l'instant, utilisées.
 

<!-- ```{r, echo=FALSE, engine='tikz', out.width='90%', fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png', fig.cap='Some caption.', engine.opts = list(template = "tikz2pdf.tex")} -->
